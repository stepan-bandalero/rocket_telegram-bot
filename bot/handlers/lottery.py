...
# from zoneinfo import ZoneInfo  # Python 3.9+, можно заменить на pytz
# 
...
# from bot.models.Lottery import Lottery
# from bot.models.LotteryTicket import LotteryTicket
# from bot.models.LotteryWinner import LotteryWinner
# 
# 
# from bot.states.create_lottery import CreateLottery
# from bot.utils.lottery import generate_lottery_preview
...
#     """
#     Парсит строку вида 'ДД.ММ.ГГГГ ЧЧ:ММ' и возвращает aware datetime
#     в часовом поясе Europe/Moscow (можно заменить на нужный).
#     """
...
#     # Локальный часовой пояс (можно взять из настроек)
...
#     await message.answer("Введите название лотереи:")
# 
# 
...
#     await state.set_state(CreateLottery.description)
#     await message.answer("Введите описание лотереи (можно отправить пустое сообщение):")
# 
# # -------------------------------------------------------------------
# # Ввод описания
# # -------------------------------------------------------------------
# @router.message(CreateLottery.description)
...
#     await message.answer("Выберите тип лотереи:", reply_markup=kb.as_markup())
# 
# # -------------------------------------------------------------------
# # Выбор типа
# # -------------------------------------------------------------------
...
#         await callback.message.answer("Введите стоимость билета в звёздах (целое число):")
#     else:
#         await state.update_data(ticket_price_stars=None)
#         await state.set_state(CreateLottery.prize_gift_id)
#         await callback.message.answer("Введите ID приза (текст):")
# 
# # -------------------------------------------------------------------
...
#     await state.set_state(CreateLottery.prize_gift_id)
#     await message.answer("Введите ID приза (текст):")
# 
# # -------------------------------------------------------------------
# # Ввод ID приза
# # -------------------------------------------------------------------
...
#     if message.from_user.id not in settings.admins:
#         return
#     await state.update_data(prize_gift_id=message.text)
#     await state.set_state(CreateLottery.winners_count)
#     await message.answer("Введите количество победителей (целое число > 0):")
# 
# # -------------------------------------------------------------------
...
#         return
#     # Дополнительно можно проверить, что дата не в прошлом
#     if dt < datetime.now(ZoneInfo("Europe/Moscow")):
#         await message.answer("Дата не может быть в прошлом. Введите будущую дату.")
#         return
#     await state.update_data(results_date=dt.isoformat())  # сохраняем как ISO строку
#     await state.set_state(CreateLottery.max_tickets_per_user)
#     await message.answer("Введите максимальное количество билетов на одного пользователя (или 0, если без ограничения):")
# 
# # -------------------------------------------------------------------
# # Ввод максимума билетов на пользователя
# # -------------------------------------------------------------------
...
#     await message.answer("Введите максимальное общее количество билетов (или 0, если без ограничения):")
# 
# # -------------------------------------------------------------------
...
# 
#     # Все данные собраны, генерируем предпросмотр
...
#         kb = InlineKeyboardBuilder()
#         kb.button(text="Да, сохранить", callback_data="confirm_yes")
#         kb.button(text="Нет, отменить", callback_data="confirm_no")
#         await state.set_state(CreateLottery.confirm)
#         await message.answer("Подтверждаете создание?", reply_markup=kb.as_markup())
...
#     data = await state.get_data()
# 
#     # Преобразуем дату обратно в datetime (из ISO строки)
...
#         tickets_sold_count=0  # по умолчанию
#     )
# 
